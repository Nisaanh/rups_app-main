<x-app-layout>
    <div class="py-2 bg-gradient-to-br from-slate-50 to-slate-100 min-h-screen">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="flex items-center justify-between">
                <div>
                    <h2 class="text-2xl font-black text-slate-800">Approval Tindak Lanjut</h2>
                    <p class="text-xs text-slate-400 mt-1">{{ $roleName }}</p>
                </div>
            ...
            <div class="grid grid-cols-3 gap-4">
                ...
                <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 flex items-center gap-4 hover:shadow-md transition">
                    ...
                    <div>
                        <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Total Ditolak</p>
            ...
            <div class="bg-white rounded-3xl shadow-lg border border-slate-100 overflow-hidden">
                <div class="p-6 bg-gradient-to-r from-slate-900 to-slate-800 border-b border-slate-700">
                    <div class="flex items-center justify-between">
                        ...
                        <span class="px-4 py-2 bg-amber-500 text-white rounded-full text-xs font-black uppercase shadow-sm">
                        ...
                        <div class="flex flex-col lg:flex-row gap-5">
                            ...
                            <div class="flex-1 min-w-0 space-y-4">
                                ...
                                <div class="flex flex-wrap items-center gap-2">
                                    ...
                                    {{-- Badge Bidang --}}
                                    <span class="px-2.5 py-1 bg-indigo-50 text-indigo-700 rounded-lg text-[9px] font-black uppercase tracking-wider border border-indigo-100">
                                        {{ $approval->tindakLanjut->arahan->bidang->name ?? 'Umum' }}
                                    </span>
                                ...
                                {{-- Arahan --}}
                                <div>
                                    <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">Arahan</p>
                                    <p class="text-sm font-bold text-indigo-700 bg-indigo-50 rounded-lg p-3 border border-indigo-100">
                            ...
                            <div class="flex lg:flex-col gap-2 flex-shrink-0 lg:w-40 items-start lg:items-stretch">
                                ...
                                <button data-id="{{ $approval->tindakLanjut->id }}" data-stage="{{ $approval->stage }}"
                                    ...
                                    class="flex-1 lg:flex-none flex items-center justify-center gap-1.5 px-4 py-2.5 bg-gradient-to-r from-indigo-600 to-sky-600 text-white rounded-xl font-bold text-[10px] uppercase tracking-wider hover:from-indigo-700 hover:to-sky-700 transition shadow-sm">
                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="bg-white rounded-3xl shadow-lg border border-slate-100 overflow-hidden">
                <div class="p-6 bg-gradient-to-r from-slate-50 to-white border-b border-slate-100">
                    <div class="flex items-center justify-between">
                        <h3 class="text-xl font-bold text-slate-800">Riwayat Keputusan</h3>
    ...
    <thead>
        <tr class="bg-slate-50/60 border-b border-slate-100">
            ...
            <th class="px-6 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest">Bidang</th>
        ...
        <tr class="hover:bg-slate-50/40 transition-colors">
            <td class="px-6 py-4">
                <p class="text-sm font-bold text-slate-800">
                    {{ $history->tindakLanjut->unitKerja->name ?? '-' }}
                </p>
    ...
    <p class="text-xs font-bold text-slate-800">
        {{ $history->tindakLanjut->arahan->bidang->name ?? 'Umum' }}
    </p>
</td>
            <td class="px-6 py-4">
                ...
                <span class="inline-flex items-center gap-1 px-2.5 py-1 bg-slate-100 text-slate-600 rounded-lg text-[9px] font-black uppercase border border-slate-200">
                    <svg class="w-2.5 h-2.5" fill="currentColor" viewBox="0 0 20 20">
                        <path fill-rule="evenodd" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636" clip-rule="evenodd" />
                    </svg>
                    TD
                </span>
                @else
            ...
            <td class="px-6 py-4 text-right">
                <a href="{{ route('approval.show', $history->tindak_lanjut_id) }}"
                    class="inline-flex items-center gap-1 px-3 py-1.5 text-[9px] font-black text-slate-500 uppercase tracking-wider hover:text-indigo-600 transition rounded-lg hover:bg-indigo-50">
    ...
    <div id="detailModal" class="fixed inset-0 bg-slate-900/70 hidden items-center justify-center z-[100] backdrop-blur-sm p-4">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden">
            ...
            <div class="overflow-y-auto flex-1 p-6 space-y-5">
                {{-- Arahan --}}
                <div>
                    <p class="text-[9px] font-black text-indigo-400 uppercase tracking-widest mb-1.5 flex items-center gap-1">
                        Arahan
                    </p>
                    <div class="bg-indigo-50 rounded-xl p-4 border border-indigo-100">
                        <p id="detailModalArahan" class="text-sm font-bold text-indigo-800 leading-relaxed">-</p>
                    </div>
                </div>
                {{-- Meta Info Grid --}}
                <div class="grid grid-cols-2 gap-4">
                    <div class="bg-slate-50 rounded-xl p-3 border border-slate-100">
                        <p class="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-0.5">Unit Kerja / Bidang</p>
                        <p class="text-xs font-bold text-slate-800">
                            <span id="detailMetaUnit">-</span>
                            <span class="text-slate-300 mx-1">•</span>
                            <span id="detailMetaBidang" class="text-indigo-600">-</span>
                        </p>
                    </div>
                    <div class="bg-slate-50 rounded-xl p-3 border border-slate-100">
                        <p class="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-0.5">Periode</p>
                        <p id="detailMetaPeriode" class="text-xs font-bold text-slate-800">-</p>
                    </div>
                    <div class="bg-slate-50 rounded-xl p-3 border border-slate-100">
                        <p class="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-0.5">Dibuat Oleh</p>
                        <p id="detailMetaCreator" class="text-xs font-bold text-slate-800">-</p>
                    </div>
                    <div class="bg-slate-50 rounded-xl p-3 border border-slate-100">
                        <p class="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-0.5">Tanggal Input</p>
                        <p id="detailMetaTanggal" class="text-xs font-bold text-slate-800">-</p>
                    </div>
                    <div class="col-span-2 bg-slate-50 rounded-xl p-3 border border-slate-100">
                        ...
                        <p id="detailMetaStatus" class="text-xs font-bold">-</p>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="bg-slate-50 rounded-xl p-4 border border-slate-100">
                        <p id="detailModalUraian" class="text-sm text-slate-700 leading-relaxed">-</p>
                    </div>
                </div>
                ...
                <div id="detailKendalaWrapper" class="hidden">
                    ...
                    <div class="bg-rose-50 rounded-xl p-4 border border-rose-100">
                        <p id="detailModalKendala" class="text-sm text-slate-700 leading-relaxed">-</p>
                    </div>
                </div>
                ...
                <div id="detailKeteranganWrapper" class="hidden">
                    ...
                    <div class="bg-blue-50 rounded-xl p-4 border border-blue-100">
                        <p id="detailModalKeterangan" class="text-sm text-slate-700 leading-relaxed">-</p>
                    </div>
                </div>

                {{-- Evidence --}}
                <div id="detailEvidenceWrapper" class="hidden">
                    <a id="detailEvidenceLink" href="#" target="_blank"
                        class="inline-flex items-center gap-2 px-4 py-2.5 bg-blue-50 text-blue-600 rounded-xl text-xs font-bold hover:bg-blue-100 transition border border-blue-100">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13" />
                        </svg>
                        Lihat Evidence
                ...
                {{-- Timeline Approval Mini --}}
                <div>
                    <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-3">Timeline Approval</p>
                    <div id="detailTimelineApproval" class="flex items-start gap-0">
                        {{-- Diisi JavaScript --}}
                    </div>
                </div>
            </div>
            ...
            <form id="approveForm" method="POST" class="p-6 space-y-4">
                ...
                <div id="noteWrapper" class="hidden">
                    ...
                    <textarea name="note" id="approveNote" rows="3"
                        ...
                        placeholder="Jelaskan alasan ditolak..."></textarea>
                </div>
    ...
    <div id="rejectModal" class="fixed inset-0 bg-slate-900/70 hidden items-center justify-center z-[100] backdrop-blur-sm p-4">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
            ...
            <div class="px-6 py-5 border-b border-rose-50 bg-rose-50/40">
                <h3 class="text-base font-black text-slate-800 uppercase tracking-tight">Catatan Ditolak</h3>
            ...
            <form id="rejectForm" method="POST" class="p-6 space-y-4">
                ...
                <div>
                    <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">
                        Alasan <span class="text-rose-500">*</span>
                    </label>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const tindakLanjutData = {
                ...
                "{{ $approval->tindakLanjut->id }}": {
                    ...
                    bidang: "{{ addslashes($approval->tindakLanjut->arahan->bidang->name ?? 'Umum') }}",
                    periode: "{{ $bulanIndonesia[$approval->tindakLanjut->periode_bulan] ?? '-' }} {{ $approval->tindakLanjut->periode_tahun }}",
                    creator: "{{ addslashes($approval->tindakLanjut->creator->name ?? '-') }}",
                    tanggal: "{{ $approval->tindakLanjut->created_at->format('d M Y H:i') }}",
                    status: "{{ $approval->tindakLanjut->status }}",
                    arahan: "{{ addslashes($approval->tindakLanjut->arahan->strategi ?? '-') }}",
                    ...
                    evidence_url: "{{ $approval->tindakLanjut->evidence_url ? Storage::url($approval->tindakLanjut->evidence_url) : '' }}",
                    approvals: [
                        @foreach($approval->tindakLanjut->approvals as $appr)
                        {
                            stage: {{ $appr->stage }},
                            status: "{{ $appr->status }}",
                            approver: "{{ addslashes($appr->approver->name ?? '-') }}",
                            note: "{{ addslashes($appr->note ?? '') }}",
                            is_td: {{ str_contains($appr->note ?? '', 'Ditetapkan sebagai TD') ? 'true' : 'false' }}
                        },
                        @endforeach
                    ]
                },
            ...
            // Update fungsi openDetailModal
            window.openDetailModal = function(id) {
                const data = tindakLanjutData[id];
                if (!data) return;

                // Header
                document.getElementById('detailModalUnit').textContent = data.unit + ' • ' + data.bidang;

                // Meta Info
                document.getElementById('detailMetaUnit').textContent = data.unit;
                document.getElementById('detailMetaBidang').textContent = data.bidang;
                document.getElementById('detailMetaPeriode').textContent = data.periode;
                document.getElementById('detailMetaCreator').textContent = data.creator;
                document.getElementById('detailMetaTanggal').textContent = data.tanggal;

                // Status badge
                const statusBadges = {
                    'approved': '<span class="px-2.5 py-1 bg-emerald-50 text-emerald-700 border border-emerald-100 rounded-lg text-[10px] font-black uppercase">✓ Selesai</span>',
                    'rejected': '<span class="px-2.5 py-1 bg-rose-50 text-rose-700 border border-rose-100 rounded-lg text-[10px] font-black uppercase">↺ Revisi</span>',
                    'in_approval': '<span class="px-2.5 py-1 bg-blue-50 text-blue-700 border border-blue-100 rounded-lg text-[10px] font-black uppercase">⏳ Proses Approval</span>',
                    'pending': '<span class="px-2.5 py-1 bg-amber-50 text-amber-700 border border-amber-100 rounded-lg text-[10px] font-black uppercase">⏰ Menunggu</span>',
                };
                document.getElementById('detailMetaStatus').innerHTML = statusBadges[data.status] || data.status;

                // Arahan
                document.getElementById('detailModalArahan').textContent = data.arahan;

                // Uraian
                document.getElementById('detailModalUraian').textContent = data.uraian;

                // Kendala
                const kendalaW = document.getElementById('detailKendalaWrapper');
                if (data.kendala.trim()) {
                    document.getElementById('detailModalKendala').textContent = data.kendala;
                    kendalaW.classList.remove('hidden');
                } else {
                    kendalaW.classList.add('hidden');
                }

                // Keterangan
                const ketW = document.getElementById('detailKeteranganWrapper');
                if (data.keterangan.trim()) {
                    document.getElementById('detailModalKeterangan').textContent = data.keterangan;
                    ketW.classList.remove('hidden');
                } else {
                    ketW.classList.add('hidden');
                }

                // Evidence
                const evW = document.getElementById('detailEvidenceWrapper');
                if (data.evidence_url) {
                    document.getElementById('detailEvidenceLink').href = data.evidence_url;
                    evW.classList.remove('hidden');
                } else {
                    evW.classList.add('hidden');
                }

                // Timeline Approval
                const stages = ['Atasan Auditi', 'Unit Kerja', 'Pengendali Teknis', 'Pengendali Mutu', 'Penanggung Jawab'];
                let timelineHtml = '';
                stages.forEach((name, i) => {
                    const stageNum = i + 1;
                    const appr = data.approvals.find(a => a.stage === stageNum);
                    const status = appr ? appr.status : 'pending';

                    let circleClass = 'bg-white border-slate-200';
                    let iconHtml = `<span class="text-[9px] font-black text-slate-400">${stageNum}</span>`;
                    let lineClass = 'bg-slate-200';

                    if (status === 'approved') {
                        circleClass = 'bg-emerald-500 border-emerald-500';
                        iconHtml = '<svg class="w-3.5 h-3.5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"/></svg>';
                        lineClass = 'bg-emerald-400';
                    } else if (status === 'rejected' && appr?.is_td) {
                        circleClass = 'bg-slate-500 border-slate-500';
                        iconHtml = '<svg class="w-3.5 h-3.5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728L5.636 5.636"/></svg>';
                        lineClass = 'bg-slate-400';
                    } else if (status === 'rejected') {
                        circleClass = 'bg-rose-500 border-rose-500';
                        iconHtml = '<svg class="w-3.5 h-3.5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M6 18L18 6M6 6l12 12"/></svg>';
                        lineClass = 'bg-slate-200';
                    }

                    const lineLeft = i > 0 ? `<div class="flex-1 h-0.5 ${lineClass}"></div>` : '';
                    const lineRight = i < 4 ? `<div class="flex-1 h-0.5 bg-slate-200"></div>` : '';

                    timelineHtml += `
            <div class="flex-1 flex flex-col items-center">
                <div class="flex items-center w-full">
                    ${lineLeft}
                    <div class="w-7 h-7 rounded-full flex-shrink-0 flex items-center justify-center border-2 ${circleClass}">${iconHtml}</div>
                    ${lineRight}
                </div>
                <p class="text-[7px] font-black text-slate-500 mt-1 text-center leading-tight">${name}</p>
            </div>`;
                });
                document.getElementById('detailTimelineApproval').innerHTML = timelineHtml;
            ...
            window.submitApproveForm = function() {
                ...
                if (selectedValue.value === 'rejected') {
                    ...
                    if (!note || note.value.trim() === '') {
                        alert('Catatan reject wajib diisi!');
                        if (note) note.focus();
                        return;
                    }
                }
